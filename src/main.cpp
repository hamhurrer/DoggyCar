...
// 定义通道参数结构体
typedef struct {
  uint16_t channel1;  // 左右转弯 (1000左-1500中-2000右)
  uint16_t channel2;  // 前进后退 (1000后-1500中-2000前)
  uint16_t channel3;  // 腿高控制 (1000低-1500中-2000高)
  uint16_t channel4;  // 翻滚控制 (1000左翻-1500中-2000右翻)
  uint16_t channel5;  // 控制模式 (>1900车轮/<1900狗子)
  uint16_t channel6;  // 车速控制 (1000慢-1500中-2000快)
  uint16_t channel7;  // 形态切换 (<1200双足/>1200狗子)
  uint16_t channel8;  // 备用通道
} RemoteChannels;

// 定义动作序列结构体（时间+通道值）
typedef struct {
  unsigned long duration;  // 该动作持续时间(毫秒)
  RemoteChannels channels; // 该时间段的通道参数
} ActionStep;

// 全局变量
RemoteChannels currentChannels;       // 当前生效的通道值
unsigned long actionStartTime = 0;    // 当前动作开始时间
int currentActionIndex = 0;           // 当前执行的动作索引

// ========== 【重点修改区】自定义动作序列 ==========
// 在这里修改动作步骤，实现不同的自动动作组合
ActionStep actionSequence[] = {
  // 步骤1：初始化-站立（持续5秒）
  {
    5000,  // 持续5000ms
    {1500, 1500, 2000, 1500, 2000, 930, 1100, 1500}
    // 通道1:居中 通道2:静止 通道3:腿高中等 通道4:居中 
    // 通道5:车轮模式 通道6:中等车速 通道7:狗子形态 通道8:备用
  // 
  },
  // // 步骤2：车形态缓慢前进（持续10秒）
   {
    10000,
    {1500, 1700, 2000, 1500, 2000, 930, 1100, 1500}
    // 通道1:居中 通道2:缓慢前进 通道3:腿高中等 通道4:居中
    // 通道5:狗子模式 通道6:中等偏快 通道7:狗子形态 通道8:备用
   },
  // // 步骤3：右转前进（持续5秒）
   {
     5000,
     {1600, 1700, 2000, 1500, 2000, 930, 1100, 1500}
     // 通道1:右转 通道2:缓慢前进 通道3:腿高中等 通道4:居中
     // 通道5:狗子模式 通道6:中等偏快 通道7:狗子形态 通道8:备用
   },
  // // 步骤4：车形态缓慢前进（持续1秒）
  {
  ...
  {
     3000,
     {1400, 1700, 2000, 1500, 2000, 930, 1100, 1500}
    // 通道1:居中 通道2:缓慢前进 通道3:腿高中等 通道4:居中
    // 通道5:狗子模式 通道6:中等偏快 通道7:狗子形态 通道8:备用
   },
   // // 步骤10：车形态缓慢前进（持续1秒）
  ...
  {
     5000,
     {1500, 1700, 2000, 1500, 1000, 1470, 2000, 1500}
    // 通道1:居中 通道2:缓慢前进 通道3:腿高中等 通道4:居中
    // 通道5:狗子模式 通道6:中等偏快 通道7:狗子形态 通道8:备用
   },
};
...
// 动作序列长度
const int actionSequenceLength = sizeof(actionSequence) / sizeof(ActionStep);
...
void initRemoteControl() {
  ...
  // 初始化为第一个动作的通道值
  currentChannels = actionSequence[0].channels;
  Serial.println("动态遥控器控制模块初始化完成");
}

/**
 * @brief 更新遥控器通道值（随时间自动切换动作）
 * @return 当前生效的通道值
 */
RemoteChannels updateRemoteControl() {
  unsigned long currentTime = millis();
  
  // 检查当前动作是否执行完毕
  if (currentTime - actionStartTime >= actionSequence[currentActionIndex].duration) {
    // 切换到下一个动作
    currentActionIndex = (currentActionIndex + 1) % actionSequenceLength;
    actionStartTime = currentTime;
    
    // 更新通道值
    currentChannels = actionSequence[currentActionIndex].channels;
    
    // 打印动作切换信息（调试用）
    Serial.printf("切换到动作 %d，持续 %d ms\n", currentActionIndex + 1, actionSequence[currentActionIndex].duration);
  }
...
/**
 * @brief 手动设置通道值（可单独调用，临时覆盖自动序列）
 * @param ch1~ch8 8个通道的数值
 */
void setRemoteChannels(uint16_t ch1, uint16_t ch2, uint16_t ch3, uint16_t ch4,
                      uint16_t ch5, uint16_t ch6, uint16_t ch7, uint16_t ch8) {
  currentChannels.channel1 = constrain(ch1, 1000, 2000);
  currentChannels.channel2 = constrain(ch2, 1000, 2000);
  currentChannels.channel3 = constrain(ch3, 1000, 2000);
  currentChannels.channel4 = constrain(ch4, 1000, 2000);
  currentChannels.channel5 = constrain(ch5, 1000, 2000);
  currentChannels.channel6 = constrain(ch6, 1000, 2000);
  currentChannels.channel7 = constrain(ch7, 1000, 2000);
  currentChannels.channel8 = constrain(ch8, 1000, 2000);
}
...
void loop()
{
  ...
  // ========== 核心修改：调用动态遥控器更新函数 ==========
  RemoteChannels currentRC = updateRemoteControl();
  
  // 将动态通道值赋值给原遥控器变量
  filteredPPMValues1 = lowPassFilter(currentRC.channel1, filteredPPMValues1, alpha);
  filteredPPMValues2 = lowPassFilter(currentRC.channel2, filteredPPMValues2, alpha);
  filteredPPMValues3 = lowPassFilter(currentRC.channel3, filteredPPMValues3, alpha);
  filteredPPMValues4 = lowPassFilter(currentRC.channel4, filteredPPMValues4, alpha);
  filteredPPMValues5 = lowPassFilter(currentRC.channel5, filteredPPMValues5, alpha);
  filteredPPMValues6 = lowPassFilter(currentRC.channel6, filteredPPMValues6, alpha);
  filteredPPMValues7 = lowPassFilter(currentRC.channel7, filteredPPMValues7, alpha);
  filteredPPMValues8 = lowPassFilter(currentRC.channel8, filteredPPMValues8, alpha);
  ...
  // filteredPPMValues5 = map(filteredPPMValues55,1050,1860,1000,1950); //机器人控制模式切换，右上摇杆，最下是车轮模式，最上是狗子模式
  ...
  // filteredPPMValues8 = map(filteredPPMValues88,1090,1890,1000,1950);
